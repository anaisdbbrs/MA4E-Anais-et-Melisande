...
class ChargingStationAgent:
    ...
    def take_decision(self, state, previous_state=None, previous_action=None, previous_reward=None):
        ...
        nb_evs = 4 # test... mettre: self.env.nb_evs
        ...
        lp.setObjective(pulp.lpSum([(l_charge[k][t] + l_decharge[k][t])* manager_signal[t]* (self.env.delta_t / H)for k in range(nb_evs)for t in range(self.nb_pdt) ]))
        ...
        for k in range(nb_evs):
            for t in range(1, self.nb_pdt):
                ...
                if k in tarr_dico_date and t == tarr_dico_date[k]:
                    liste_soc[k][t-1] -= 4
        print(f"soc reconstitué{liste_soc}")

        soc_code = np.zeros((nb_evs, self.nb_pdt))
        for k in range(nb_evs):
            for t in range(0, self.nb_pdt):
                soc_code[k][t] = a[k][t].value()
        print(f"soc du code{soc_code}")
        ...
        print(f"Dict depart {tdep_dico_date}")
        print(f"Dict arrivée {tarr_dico_date}")
...
if __name__ == "__main__":
    ...
    station_config = {"pmax": 40, "evs": evs_config}
    ...
    for i in range(N * 2):
        ...
        if done:
            break
        print(
            f"action: {action}, reward: {reward}, cumulative reward: {cumulative_reward}"
        )
